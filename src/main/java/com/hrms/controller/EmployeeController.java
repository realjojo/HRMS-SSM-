...
public class EmployeeController {
    ...
    public JsonMsg deleteEmp(@PathVariable("empId") Integer empId){
        ...
        if (res != 1){
            return JsonMsg.fail().addInfo("employee_del_error", "删除员工信息异常");
        }
    ...
    public JsonMsg updateEmp(@PathVariable("empId") Integer empId, Employee employee) {
        ...
        if(res != 1) {
            return JsonMsg.fail().addInfo("employee_update_error", "更改员工信息异常");
        }
        return JsonMsg.success();
    }

    @RequestMapping(value = "/insertEmp", method = RequestMethod.POST)
    public JsonMsg insertEmp(Employee employee) {
        int res = employeeService.insertEmp(employee);
        if(res != 1) {
            return JsonMsg.fail().addInfo("employee_insert_error", "新增员工信息异常");
        }
        return JsonMsg.success();
    }
